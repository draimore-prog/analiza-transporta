...
export function FieldWorkOrderForm({
  ...
  const STEPS = [
    { id: 1, title: "1. Jedinica", subtitle: "Odabir & zadatak" },
    { id: 2, title: "2. Ček-lista", subtitle: `${defectCount > 0 ? `${defectCount} defekt` : "Ispravnost"}` },
    { id: 3, title: "3. Radovi", subtitle: "Opis & dijelovi" },
    { id: 4, title: "4. Sati & slike", subtitle: `${photoCount}/5 slika` }
  ];

  return (
    <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-md flex justify-center items-center z-[9995] p-2 sm:p-4 animate-in fade-in">
      <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-2xl max-h-[96vh] flex flex-col overflow-hidden border border-slate-200 dark:border-slate-800">
        
        {/* Mobile-First Zaglavlje sa naslovom i X */}
        ...
        {/* STEPPER NAVIGATOR (4 ZASEBNA KORAKA) */}
        <div className="bg-slate-100 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 px-3 py-2.5 shrink-0">
            ...
            {STEPS.map((s) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    // Dozvoli skakanje unazad ili na sljedeći korak ako je mašina odabrana
                    if (s.id < currentStep) {
                      setCurrentStep(s.id);
                      setErrorMessage("");
                    } else if (s.id > currentStep && selectedVehicle) {
                      setCurrentStep(s.id);
                      setErrorMessage("");
                    }
                  }}
                  className={`py-2 px-1.5 rounded-xl text-center transition-all cursor-pointer border flex flex-col items-center justify-center ${
                    isActive
                      ...
                      : "bg-white dark:bg-slate-850 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-700 opacity-75"
                  }`}
                >
                  <div className="flex items-center gap-1">
                    {isPast && <Check className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" />}
                    <span className="text-[11px] sm:text-xs font-black truncate">{s.title}</span>
                  </div>
                  <span className="text-[9px] sm:text-[10px] font-semibold opacity-90 truncate hidden xs:inline">
                    {s.subtitle}
                  </span>
                </button>
              );
        ...
        {/* TIJELO TRENUTNE STRANICE / KORAKA (NEMA BESKONAČNOG SCROLLA) */}
        <div className="p-4 sm:p-6 overflow-y-auto flex-1 space-y-4 text-slate-800 dark:text-slate-200">
          {/* Upozorenje o grešci */}
          {errorMessage && (
            <div className="p-3 bg-red-50 dark:bg-red-950/60 border border-red-200 dark:border-red-900 text-red-700 dark:text-red-300 text-xs rounded-xl font-bold flex items-center gap-2 animate-in fade-in">
            ...
            <div className="space-y-4 animate-in fade-in">
              <div className="bg-slate-50 dark:bg-slate-850 p-4 sm:p-5 rounded-2xl border border-slate-200 dark:border-slate-700/80 space-y-3">
                ...
                {isAssignedOrder ? (
                  ...
                  <div className="p-4 sm:p-5 bg-amber-50 dark:bg-amber-950/60 border-2 border-amber-400 dark:border-amber-600 rounded-2xl shadow-sm space-y-2.5">
                    <div className="flex items-center gap-2 text-amber-900 dark:text-amber-200 text-xs font-black uppercase tracking-wider">
                    ...
                    <div className="text-xs sm:text-sm text-slate-600 dark:text-slate-400 flex flex-wrap items-center gap-x-3 gap-y-1">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="max-h-56 overflow-y-auto divide-y divide-slate-100 dark:divide-slate-700 border border-slate-200 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-800">
              ...
              {/* Vrsta intervencije */}
              <div className="bg-slate-50 dark:bg-slate-850 p-4 rounded-2xl border border-slate-200 dark:border-slate-700/80 space-y-2">
          ...
          {/* ========================================================================= */}
          {/* KORAK 2: KONTROLNA ČEK-LISTA (8 SKLOPOVA)                                */}
          {/* ========================================================================= */}
          {currentStep === 2 && (
            <div className="space-y-3.5 animate-in fade-in">
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-[11px] text-slate-500 dark:text-slate-400">
                    Pregledajte sve sklopove mehanizacije i zabilježite uočene defekte.
                  </p>
                </div>
                {defectCount > 0 ? (
                  <span className="text-xs font-black bg-rose-100 text-rose-800 dark:bg-rose-950 dark:text-rose-300 px-2.5 py-1 rounded-lg border border-rose-300">
                    ⚠️ {defectCount} defekt(a)
                  </span>
                ) : (
                  <span className="text-xs font-black bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300 px-2.5 py-1 rounded-lg border border-emerald-300">
                    ✓ Sve ispravno
                  </span>
                )}
              </div>
              ...
              <button
                ...
                className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 active:scale-98 text-white rounded-xl text-xs sm:text-sm font-black flex items-center justify-center gap-2 shadow-sm cursor-pointer transition-all"
              >
            ...
            <div className="space-y-4 animate-in fade-in">
              <div className="space-y-2.5">
                <label className="block text-sm font-black uppercase text-slate-800 dark:text-slate-200 tracking-wider flex items-center gap-1.5">
                  ...
                  Opis izvršenih radova / servisne intervencije
                </label>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {COMMON_REPAIR_TEMPLATES.map((tmpl) => (
                    <button
                      ...
                      className="text-[11px] font-bold bg-slate-100 hover:bg-blue-50 dark:bg-slate-800 dark:hover:bg-blue-950 text-slate-800 dark:text-slate-200 hover:text-blue-700 border border-slate-300 dark:border-slate-700 px-2.5 py-1 rounded-xl transition-colors cursor-pointer shadow-2xs"
                    >
                      + {tmpl}
                    </button>
                  ))}
                ...
                <textarea
                  ...
                  rows={4}
                  placeholder="Napišite detaljan opis šta je pregledano ili popravljeno na viljuškaru..."
          ...
          {currentStep === 4 && (
            <div className="space-y-4 animate-in fade-in">
              {/* Radni Sati (MTH) */}
              <div className="bg-amber-50 dark:bg-amber-950/40 p-4 rounded-2xl border-2 border-amber-400 dark:border-amber-600 space-y-2">
                <label className="block text-sm font-black uppercase text-slate-800 dark:text-slate-200 flex items-center justify-between">
                  <span className="flex items-center gap-1.5">
                    <Clock className="w-5 h-5 text-amber-600" />
                    Radni sati sa table (MTH)
                  </span>
                  <span className="text-rose-600 font-black text-xs uppercase bg-rose-100 dark:bg-rose-950 px-2.5 py-0.5 rounded-md">
                    * Obavezno
                  </span>
                </label>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-white dark:bg-slate-900 border-2 border-amber-500 rounded-2xl pl-11 pr-16 py-3.5 text-2xl font-mono font-black text-slate-900 dark:text-white outline-none focus:ring-4 focus:ring-amber-500/20 shadow-inner"
                  />
                ...
                <p className="text-[11px] text-amber-900 dark:text-amber-300 font-medium">
                  Upišite tačan broj radnih sati sa digitalnog ili analognog brojača viljuškara.
                </p>
              </div>

              {/* Fotodokumentacija sa 5 pozicija */}
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400">
                      Slikajte viljuškar direktno kamerom telefona. Slike se automatski optimizuju.
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-[11px] font-black uppercase px-2.5 py-1 rounded-lg bg-rose-100 text-rose-700 dark:bg-rose-950 dark:text-rose-300 border border-rose-300">
                      * Min. 1 slika
                    </span>
                    <span className="text-[11px] font-black px-2 py-1 rounded-lg bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300 border border-emerald-300">
                      {photoCount}/5 slika
                    </span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 max-h-[36vh] overflow-y-auto pr-1">
                  {PHOTO_SLOTS.map((slot) => {
                    ...
                    return (
                      <div
                        key={slot.key}
                        className="p-3 bg-slate-50 dark:bg-slate-850 rounded-2xl border-2 border-slate-200 dark:border-slate-700 flex flex-col justify-between"
                      >
                        <div className="flex justify-between items-start mb-1.5">
                          <div>
                            <span className="font-black text-xs text-slate-900 dark:text-white block">{slot.label}</span>
                            <span className="text-[10px] text-slate-500 dark:text-slate-400 leading-tight">{slot.hint}</span>
                          </div>
                          {imgData && (
                        ...
                        {imgData ? (
                          <div className="relative rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700 mb-1 group">
                            <img src={imgData} alt={slot.label} className="w-full h-28 object-cover" />
                            <button
                              type="button"
                              onClick={() => handleRemovePhoto(slot.key)}
                              className="absolute top-2 right-2 p-1.5 bg-rose-600 hover:bg-rose-700 text-white rounded-xl shadow-md cursor-pointer"
                          ...
                          <div
                            ...
                            className="h-24 border-2 border-dashed border-slate-300 dark:border-slate-600 hover:border-blue-500 rounded-xl flex flex-col items-center justify-center p-2 text-center cursor-pointer transition-colors bg-white dark:bg-slate-800 active:scale-98"
                          >
                            {isCompressing ? (
                              <div className="flex flex-col items-center gap-1.5 text-blue-600">
                                <div className="w-5 h-5 border-2 border-blue-600 border-t-transparent rounded-full animate-spin" />
                                <span className="text-[11px] font-bold">Kompresija...</span>
                              </div>
                            ) : (
                              <>
                                <Camera className="w-6 h-6 text-indigo-500 mb-0.5" />
        ...
        <div className="p-3 sm:p-4 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex items-center justify-between gap-2 shrink-0">
          {/* Dugme lijevo: Nazad ili Odustani */}
          {currentStep > 1 ? (
            <button
              type="button"
              onClick={handlePrevStep}
              className="px-4 sm:px-5 py-3 rounded-xl text-xs sm:text-sm font-black bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 transition-all flex items-center gap-1.5 cursor-pointer"
            >
            ...
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-3 rounded-xl text-xs sm:text-sm font-bold text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer"
            >
              Odustani
            </button>
          )}
          ...
          {currentStep < 4 ? (
            <button
              ...
              className="px-6 py-3.5 bg-blue-600 hover:bg-blue-700 active:scale-98 text-white text-xs sm:text-sm font-black rounded-xl shadow-md transition-all flex items-center gap-2 cursor-pointer ml-auto"
            >
              <span>
            ...
            <button
              ...
              className="px-6 sm:px-8 py-3.5 bg-emerald-600 hover:bg-emerald-700 active:scale-98 text-white text-xs sm:text-sm font-black rounded-xl shadow-lg transition-all flex items-center gap-2 cursor-pointer disabled:opacity-50 ml-auto"
            >
              {isSubmitting ? (
