...
export function ServiserDashboard({
  ...
  // Kombinovana flota (drumska vozila + skladišna mehanizacija za kompletnu pretragu)
  const combinedFleet = useMemo(() => {
    const seen = new Set();
    const list = [];

    // Prvo dodaj drumska vozila
    masterFleet.forEach((v) => {
      const key = (v.reg || "").trim().toUpperCase();
      if (key && !seen.has(key)) {
        seen.add(key);
        list.push(v);
      }
    ...
    // Zatim dodaj skladišnu mehanizaciju
    warehouseMasterFleet.forEach((v) => {
      const key = (v.reg || v.id || "").trim().toUpperCase();
      if (key && !seen.has(key)) {
        seen.add(key);
        list.push({
          ...v,
          reg: v.reg || v.id,
          tipMehan: v.tipMehan || v.kategorija || "Skladišna mehanizacija"
        });
      }
    });

    return list;
  }, [masterFleet, warehouseMasterFleet]);

  // Filtriranje vozila po reg, garaznom broju, marki, modelu, sasiji
  const filteredVehicles = useMemo(() => {
    const term = searchTerm.trim().toLowerCase();
    if (!term) {
      return combinedFleet.filter((v) => (v.status || "Aktivno").toLowerCase() === "aktivno").slice(0, 10);
    }
  ...
  const getVehicleIcon = (tip) => {
    ...
    if (t.includes("skladi") || t.includes("viljuš") || t.includes("viljusk") || t.includes("paletar")) return "🚜";
    return "🚛";
  };

  const totalUnits = combinedFleet.length || masterFleet.length;
  const totalCostRecords = (costData.length || 0) + (warehouseCostData?.length || 0);

  return (
    <div className="min-h-screen w-full bg-slate-100 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col justify-between overflow-y-auto">
      {/* Gornja traka */}
      <header className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-4 sm:px-6 py-4 flex flex-wrap items-center justify-between gap-3 shadow-xs relative z-30">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {onNavigateToField && (
            <button
              ...
              className="text-xs font-bold bg-emerald-50 hover:bg-emerald-100 text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-300 dark:hover:bg-emerald-900/60 px-3 py-1.5 rounded-xl transition-all flex items-center gap-1.5 border border-emerald-200 dark:border-emerald-800 cursor-pointer"
          ...
          <div className="flex items-center gap-2 bg-slate-50 dark:bg-slate-800/80 px-2.5 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700 text-xs">
            ...
            <span className="font-extrabold text-slate-800 dark:text-slate-200 truncate max-w-[130px]">
          ...
          {onLogout && (
            <button
              type="button"
              onClick={onLogout}
              className="text-xs font-bold bg-red-50 hover:bg-red-100 text-red-700 dark:bg-red-950/40 dark:text-red-300 dark:hover:bg-red-900/50 px-2.5 py-1.5 rounded-xl transition-all flex items-center gap-1.5 border border-red-200 dark:border-red-900/60 cursor-pointer"
