...
import { X, Download, Layers, Search, ChevronLeft, ChevronRight, Printer } from "lucide-react";
...
export function SegmentDetailModal({
  ...
  const distinctVehicles = useMemo(() => {
    return new Set(segmentTransactions.map((c) => c.reg).filter((r) => r && r !== "-")).size;
  }, [segmentTransactions]);
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/80 flex justify-center items-center z-[75] backdrop-blur-xs p-3 sm:p-5 animate-in fade-in zoom-in-95 duration-200">
      <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-5xl max-h-[94vh] flex flex-col overflow-hidden border border-slate-200 dark:border-slate-800">
        {/* Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-emerald-200 mt-0.5">
                Kompletan spisak svih faktura, servisa i troškova u ovom segmentu
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
        ...
        <div className="p-6 overflow-y-auto space-y-6 bg-slate-50 dark:bg-slate-900/50 text-xs flex-1">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="bg-white dark:bg-slate-800 p-3.5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xs">
              <span className="text-[10px] font-extrabold uppercase text-slate-400">Ukupan Trošak Segmenta</span>
              <h3 className="text-lg font-black text-emerald-900 dark:text-emerald-300 mt-1">
                {formatKM(totalCost)}
              </h3>
            </div>
            <div className="bg-white dark:bg-slate-800 p-3.5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xs">
              <span className="text-[10px] font-extrabold uppercase text-slate-400">Ukupno Intervencija</span>
          ...
          <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xs">
            <span className="text-[10px] font-extrabold uppercase text-slate-400 block mb-2">
              Utrošak po Godinama za segment "{segmentName}":
            </span>
                ...
                <div
                  ...
                  className={`p-2.5 rounded-lg border text-center cursor-pointer transition-all ${
                    selectedYear === y.toString()
                      ...
                      : "bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-700 hover:border-emerald-400"
                  }`}
                >
            ...
            <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
              ...
              <div className="relative flex-1 sm:w-56">
                <input
                  ...
                  placeholder="🔍 Pretraži reg, opis, servisera..."
