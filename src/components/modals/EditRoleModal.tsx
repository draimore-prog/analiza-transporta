...
import { AppRole, NavigationPanel, RolePermissions } from "@/types/roles";
...
interface EditRoleModalProps {
  ...
  onClose: () => void;
  role: AppRole | null;
  onSaveRole: (role: AppRole) => Promise<void>;
}

const ALL_AVAILABLE_PANELS: NavigationPanel[] = [
...
export function EditRoleModal({
  ...
  const [defaultPortal, setDefaultPortal] = useState<"transport" | "warehouse" | "serviser">("transport");
  ...
  useEffect(() => {
    if (role) {
      ...
      setPermissions({ ...((role.permissions as unknown as Record<string, boolean>) || {}) });
    }
  }, [role]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const allowedPortalsSet = new Set<"transport" | "warehouse" | "serviser">();
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/80 flex justify-center items-center z-[80] backdrop-blur-xs p-4">
      <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-2xl max-h-[92vh] flex flex-col overflow-hidden border border-slate-200 dark:border-slate-800 animate-in fade-in zoom-in-95 duration-200">
        {/* Header */}
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-5 text-xs flex-1 bg-slate-50 dark:bg-slate-900/50">
          ...
          <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xs grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div>
              ...
              <select
                value={defaultPortal}
                onChange={(e) => setDefaultPortal(e.target.value as any)}
                className="w-full border border-slate-300 dark:border-slate-600 rounded-lg p-2.5 font-bold outline-none focus:ring-2 focus:ring-indigo-500 bg-white dark:bg-slate-900 text-slate-900 dark:text-white"
              >
