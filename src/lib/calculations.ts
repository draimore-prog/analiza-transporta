import { Vehicle, VehicleCategory } from "@/types/fleet";
import { CostItem } from "@/types/cost";
import { LOCKED_2026_BASELINE } from "./constants";

export function cleanVehicleType(tipRaw?: string): VehicleCategory {
...
export function cleanBrandName(brandRaw?: string): string {
  if (!brandRaw) return '-';
...
export function calculateFleetByYear(masterFleet: Vehicle[]): Record<number, number> {
...
export function calculateDynamic2026(masterFleet: Vehicle[]) {
